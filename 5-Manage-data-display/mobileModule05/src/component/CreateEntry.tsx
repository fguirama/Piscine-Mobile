    ...
    return (<Modal visible={visible} animationType="fade" transparent onRequestClose={onClose}>
        <Pressable className="flex-1 justify-center items-center bg-black/50 px-6" onPress={onClose}>
            <Pressable className="w-full bg-white rounded-3xl p-6" onPress={(e) => e.stopPropagation()}>
                ...
                <TextInput value={title} onChangeText={setTitle} placeholder="Give your day a title..." className="border border-gray-200 rounded-xl px-4 py-3 mb-5"/>

                <Text className="font-semibold text-gray-700 mb-2">How do you feel?</Text>
                ...
                <Text className="font-semibold text-gray-700 mb-2">Content</Text>
                {errorContent && <ErrorMsg>{errorContent}</ErrorMsg>}
                <TextInput value={content} onChangeText={setContent} placeholder="Write about your day..." multiline textAlignVertical="top" className="border border-gray-200 rounded-xl px-4 py-3 h-40 mb-6"/>
